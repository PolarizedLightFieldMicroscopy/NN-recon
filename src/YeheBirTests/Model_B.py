...
class B13Net(nn.Module):
    ...
    def _swap_input(self, input, mode=None):
        assert input is not None, "Input is None"
        inputs = []
        if mode is None:  # return the exact input, (0, 1, 2, 3) is the orignal order
            inputs.append(input)  # (0, 1, 2, 3)
        elif mode == "swapD1D2":  # return 2 inputs with swaped XY_1 and XY_2
            inputs.append(input)  # (0, 1, 2, 3)
            input.swapaxes(-1, -3)  # (0, 3, 2, 1)
            inputs.append(input.swapaxes(-2, -4))  # (2, 3, 0, 1)
        elif mode == "swapX1":  # return n inputs with each dimension swapped to X_1
            inputs.append(input)  # (0, 1, 2, 3)
            inputs.append(input.swapaxes(-3, -4))  # (1, 0, 2, 3)
            inputs.append(input.swapaxes(-2, -4))  # (2, 0, 1, 3)
            inputs.append(input.swapaxes(-1, -4))  # (3, 0, 1, 2)
        else:
...
class B13NetV1(B13Net):
    ...
    def forward(self, input):
        inputsA = self._swap_input(input, mode="swapD1D2")
