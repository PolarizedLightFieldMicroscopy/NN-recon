...
def plot_lines(delta_n, optic_axis, fig=None, draw_spheres=True):
    ...
    coords_tip = [
        (coords[i] + 0.5 + optic_axis[i, ...] * delta_n * 0.75) * volume_size_um[i]
        for i in range(3)
    ]
    ...
    all_color[all_color != 0] -= all_color[
        all_color != 0
    ].min()  # twice the length of the nonzero delta_n
    all_color += 0.5
...
def plot_surface_lines(delta_n, optic_axis):
    ...
    fig = plot_lines(delta_n, optic_axis, fig=fig, draw_spheres=False)
    return fig
